...
public class GettheWeather {
	...
	public static void getWeather(weatherBean wBean) throws IOException {
		String API_KEY = "11c26ed57e8884f31b500ce989c4777d";
		...
		// print and test in a browser clouds
		...
		for (int i = 0; i < lastupdateList.getLength(); i++) {
			Node lastupdateNode = lastupdateList.item(i);
			if (lastupdateNode.getNodeType() == Node.ELEMENT_NODE) {
				Element lastupdateElement = (Element) lastupdateNode;
				String XMLlastupdate = lastupdateElement.getAttribute("value");
				System.out.println(wBean.getCityStr() + " " + XMLlastupdate);
				wBean.setLastupdateStr(XMLlastupdate);
			}
		}

		// WINDSPEED
		NodeList windSpeedList = doc.getElementsByTagName("speed");

		for (int j = 0; j < windSpeedList.getLength(); j++) {
			Node windSpeedNode = windSpeedList.item(j);
			if (windSpeedNode.getNodeType() == Node.ELEMENT_NODE) {
				Element windSpeedElement = (Element) windSpeedNode;
				String XMLwindSpeed = windSpeedElement.getAttribute("value");
				System.out.println(wBean.getCityStr() + " " + XMLwindSpeed);
				wBean.setWindSpeedStr(XMLwindSpeed);
			}
		}
	}
